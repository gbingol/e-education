...
<%!
   ...
   final String TableName="PSYCHROMETRICCALCULATION1";
...
<%
  ...
  if(!IsValuesGiven){
		...
		T=(double)(int)(20+Math.random()*10);
		Tyas=T-(double)(int)(Math.random()*5)-5;
		P=(double)(int)(95+Math.random()*10);     //kPa
		//
		varP=P*1000;
		varPwsyas=pf.Pwsyas(Tyas);
		varPws=pf.Pws(T);
		varWsyas=pf.Wsyas(varP,varPwsyas);
		varW=pf.W4(varWsyas,T,Tyas);
        ...
        RH=varRH*100;                       //%RH
		//
       values[0]=P;values[1]=T;values[2]=Tyas;
       //
       //
   ...
   dbc.updateString("Number",username,"SystemAnswer1",TableName,String.valueOf(W));
   dbc.updateString("Number",username,"SystemAnswer2",TableName,String.valueOf(RH));
 }else{
   for(int i=0;i<values.length;i++){
  ...
  dbc.close();
  P=values[0];T=values[1];Tyas=values[2];
}
...
%>
<P align=center><font color="#ff0080" size="5" face="Comic Sans MS"><STRONG>Psychrometric 
  Calculation -1</STRONG></font></P>
<P align="justify"><font face="Comic Sans MS">Given </font><font face="Comic Sans MS"> 
  Dry Bulb Temperature=<%=T%>�C, Wet Bulb Temperature=<%=Tyas%>�C and Pressure=<%=P%> (kPa) find,</font></P>
<P align="justify"><font face="Comic Sans MS"> A) Absolute Humidity (g su/kg dry 
  air),<br>
  B) Relative Humidity (%) .</font></P>
<P align=center><font face="Comic Sans MS"><STRONG>ANSWERING SECTION</STRONG></font></P>
<FORM action="<%=response.encodeURL("psikrometritablo.jsp")%>" method="POST">
  <P align=center> 
  <TABLE style="HEIGHT: 61px" cellSpacing=1 cellPadding=1 width=431 
border=1>
    <TBODY bgColor=aqua>
      <TR> 
        <TD width=161><font face="Comic Sans MS">Absolute Humidity=</font></TD>
        <TD width="156" align=middle><font face="Comic Sans MS">
          <INPUT name=useranswer0>
          </font></TD>
        <TD width=96><font face="Comic Sans MS">g su/kg da</font></TD>
      </TR>
	  <TR> 
        <TD width=161><font face="Comic Sans MS">Relative Humidity=</font></TD>
        <TD width="156" align=middle><font face="Comic Sans MS">
          <INPUT name=useranswer1>
          </font></TD>
        <TD width=96><font face="Comic Sans MS">%</font></TD>
      </TR>
      <TR> 
        <TD colSpan=3> <P 
      align=center> <font face="Comic Sans MS">
            <INPUT type=submit value=EVALUATE name=submit1>
            </font></P></TD>
      </TR>
      <TR> 
        <TD colSpan=3><P></P></TD>
      ...
      <TR> 
        <TD width=161><font face="Comic Sans MS">Value of question=</font></TD>
        <TD align=middle><font face="Comic Sans MS"><%=strvalueforquestion%></font></TD>
        <TD width=96><font face="Comic Sans MS">points</font></TD>
      </TR>
    </TBODY>
  </TABLE>
...
class Grade
{
    void GradeIt(JspWriter out,String[] results)
 {
	  ...
	  double varRH=Double.valueOf(dbc.getSelectedCell(username,"SystemAnswer2",TableName)).doubleValue ();
      ...
      try{
        if(!isanswered){
		  ...
		  lowerlimit[1]=varRH*(1-QuesRange);upperlimit[1]=varRH*(1+QuesRange);
          //
