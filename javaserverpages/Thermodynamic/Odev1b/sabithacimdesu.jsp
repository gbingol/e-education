...
<%!
   ...
   //threads except newly created classes like results
	...
	/*
	**1b. Odev 1. Soru----> MSAccess'teki Tablosu Odev1b_1
	**Yunus �engelin kitab�ndaki 2-4 Sayfa:50
	*/
   final String TableName="Odev1b_1";
...
%>
<%
  double T,m,msivi,P,V;
  boolean IsValuesGiven=dbc.IsValueTaken("IsValueTaken","STUDENTNUMBER",TableName,username);
  if(!IsValuesGiven){
		double x=0,mbuhar=0,vf=0,vfg=0;
  		T=(double)(int)(Math.random()*100+100);
        m=(double)(int)(Math.random()*10+10);
        msivi=(double)(int)(m-Math.random()*5-2);
		//
		jspclass.JFluids.SuOzellikleri su=new jspclass.JFluids.SuOzellikleri();
        P=su.P(T);  //basinc degeri bulunur kPa
        mbuhar=m-msivi;
        x=mbuhar/m;
        double v=su.vf(T)+x*(su.vg(T)-su.vf(T));
        V=m*v;
		//
       values[0]=T;values[1]=m;values[2]=msivi;
      //
      //
   ...
   dbc.updateString("STUDENTNUMBER",username,"SystemAnswer1",TableName,String.valueOf(P));
  ...
  dbc.close();
  T=values[0];m=values[1];msivi=values[2];
}
...
<P align=center><font face="Comic Sans MS"><STRONG><FONT size=5 color="#ff0080">Sabit 
  Hacimde Su</FONT></STRONG></font></P>
<P align="justify"><font face="Comic Sans MS">Sabit hacimli kapal� bir kapta <%=T%>�C 
  s�cakl�kta <%=m%> (kg) su bulunmaktad�r. E�er suyun <%=msivi%> (kg) s�v� geri 
  kalan� buhar faz�nda ise,</font></P>
<P align="justify"><font face="Comic Sans MS"> A) Kaptaki bas�nc� (kPa),<br>
  B) Kab�n hacmini (m<sup>3</sup>)</font></P>
<P align=center><font face="Comic Sans MS"><STRONG>CEVAPLAMA B�L�M�</STRONG></font></P>
<FORM action="<%=response.encodeURL("sabithacimdesu.jsp")%>" method="POST" onSubmit="return checkDigitSeperator(this)">
  <P align=center> 
  <TABLE style="HEIGHT: 61px" cellSpacing=1 cellPadding=1 width=379 
border=1>
    <TBODY bgColor=aqua>
      <TR> 
        <TD width=184><font face="Comic Sans MS">Kaptaki bas�n�</font></TD>
	  ...
	  <TR> 
        <TD width=184><font face="Comic Sans MS">Kab�n Hacmi=</font></TD>
        <TD width="144" align=middle><font face="Comic Sans MS">
...
class Grade
{
    void GradeIt(JspWriter out,String[] results)
 {
      ...
      double varP=Double.valueOf(dbc.getSelectedCell(username,"SystemAnswer1",TableName)).doubleValue ();
      ...
      try{
        if(!isanswered){
          ...
          //
          lowerlimit[0]=varP*(1-QuesRange);upperlimit[0]=varP*(1+QuesRange);
