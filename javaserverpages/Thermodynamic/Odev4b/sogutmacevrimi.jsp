...
<%!
   ...
   final String TableName="Odev4b_1";
  ...
  if(!IsValuesGiven){
   ...
   for(int j=1;j<=5;j++){
     for(int i=0;i<values.length;i++){
       if(j==2) values[1]=values[1]+5;
       if(j==3) values[3]=values[3]-5;
       if(j==4) values[0]=values[0]-10;
       if(j==5) values[2]=values[2]+10;
       String ColumnToSet="Value"+String.valueOf(i+1);
       String TableToSet="Odev4b_"+String.valueOf(j);
	   //Insert names into table then insert their values
		String strusername=dbc.getSelectedCell(username,"STUDENTNUMBER",TableToSet);
       ...
       //Reset values to their original values back
       if(j==2) values[1]=values[1]-5;
       if(j==3) values[3]=values[3]+5;
       if(j==4) values[0]=values[0]+10;
       if(j==5) values[2]=values[2]-10;
...
<FORM action="<%=response.encodeURL("sogutmacevrimi.jsp")%>" method="POST" onSubmit="return checkDigitSeperator(this)">
  <P align=center> 
