...
<%!
   ...
   final String TableName="Odev1a_4";
...
%>
<%
  double m,A,Pyerel,Psilindir;
  boolean IsValuesGiven=dbc.IsValueTaken("IsValueTaken","STUDENTNUMBER",TableName,username);
  if(!IsValuesGiven){
   //do{
    	m=(double)(int)(Math.random()*50+50);
        A=(double)(int)(Math.random()*30+20);
        Pyerel=(double)(int)(Math.random()*10+95);
		double varA=A/10000,varPyerel=Pyerel/100; //cm2--->m2   kPa--->Bar
        Psilindir=varPyerel+(m*9.8/varA)*1/Math.pow(10,5);  //Psilindir=Patm+mg/(A*10^5)
   //}while(!((Ro>0.5)&&(Ro<0.95)));
   //
   values[0]=m;values[1]=A;values[2]=Pyerel;
      //
      //
   ...
   dbc.updateString("STUDENTNUMBER",username,"SystemAnswer1",TableName,String.valueOf(Psilindir));
 }else{
   for(int i=0;i<values.length;i++){
  ...
  dbc.close();
  m=values[0];A=values[1];Pyerel=values[2];
}
...
%>
<P align=center><font face="Comic Sans MS"><STRONG><FONT size=5 color="#ff0080">P�STON</FONT></STRONG></font></P>
<font face="Comic Sans MS">Kitab�n�zda �ekil 1-41'de g�sterilen piston-silindir 
d�zene�inde bir gaz bulunmaktad�r. Pistonun k�tlesi <%=m%> (kg) olup, kesit alan� <%=A%> 
(cm<sup>2</sup>)'dir. Yerel atmosfer bas�nc� <%=Pyerel%> (kPa) ve yer�ekimi ivmesi 9.8 m/s<sup>2</sup> oldu�una 
g�re silindir i�indeki gaz�n bas�nc� (Bar) ne kadard�r. </font> 
<P align="justify">&nbsp;</P>
<P align=center><font face="Comic Sans MS"><STRONG>CEVAPLAMA B�L�M�</STRONG></font></P>
<FORM action="<%=response.encodeURL("piston.jsp")%>" method="POST" onSubmit="return checkDigitSeperator(this)">
  <P align=center> 
  <TABLE style="HEIGHT: 61px" cellSpacing=1 cellPadding=1 width=379 
border=1>
    <TBODY bgColor=aqua>
      <TR> 
        <TD width=184><font face="Comic Sans MS">Gaz�n bas�nc�</font></TD>
        <TD width="144" align=middle><font face="Comic Sans MS">
          <INPUT name=useranswer0>
          </font></TD>
        <TD width=33><font face="Comic Sans MS">Bar</font></TD>
      </TR>
      <TR> 
        <TD colSpan=3> <P 
...
class Grade
{
    void GradeIt(JspWriter out,String[] results)
 {
      ...
      double varPsilindir=Double.valueOf(dbc.getSelectedCell(username,"SystemAnswer1",TableName)).doubleValue ();
      ...
      try{
        if(!isanswered){
          ...
          //
          lowerlimit[0]=varPsilindir*(1-QuesRange);upperlimit[0]=varPsilindir*(1+QuesRange);
          //
