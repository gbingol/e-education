...
<%!
   ...
   final String TableName="3_SILINDIRIKMETALELEMAN";
...
<%
  ...
  final double kAl = 500,CpAl=320,RoAl = 8500,T0=100,Tsonsuz=50,T = 75;
  ...
  double r=0,Re=0;
       do{
          do{
            D =(double)(int)(Math.random() * 5 + 1);   //Metal eleman�n �ap� (cm)
            L =(double)(int)(Math.random() * 10 + 1);   //Metal eleman�n uzunlu�u (cm)
            V =(double)(int)(Math.random() * 5 + 1);    //Akan suyun h�z� (m/s)
            r = (D/2) / 100;
            Re = (Ro*V*D/100)/Viskozite;
          }while(!((Re > 0.4) && (Re < 400000)));
          h = heatfunc.H_CylinderandSphere((D/100), k, Re, Pr);
          Volume = Math.PI *Math.pow(r,2)*L/100;
          A = 2 * Math.PI * r * (r + L/100);
          Biot = (h * (Volume / A)) / kAl;
          Temp1 = h * A / (RoAl * Volume * CpAl);
          Temp2 = (T - Tsonsuz) / (T0 - Tsonsuz);
		  time=-Math.log(Temp2)/Temp1;
       }while(!(Biot<0.05));	//Normalde 0.1 olan fark g�venlik i�in 1/10 oran�nda d���r�lm��t�r
       values[0]=D;values[1]=L;values[2]=V;
      //
	  ...
	  try
		...
		outData .writeChars("Hacim="+String.valueOf(Volume)+"\n");
		outData .writeChars("Alan="+String.valueOf(A)+"\n");
		outData .writeChars("Biot="+String.valueOf(Biot)+"\n");
		outData.close();
		fo.close();
   ...
   dbc.updateString("STUDENTNUMBER",username,"SystemAnswer1",TableName,String.valueOf(time));
 }else{
   for(int i=0;i<values.length;i++){
  ...
  dbc.close();
  D=values[0];L=values[1];V=values[2];
}
...
%>
<P align=center><font face="Comic Sans MS"><STRONG><FONT size=5 color="#ff0080">S�L�ND�R�K 
  METAL ELEMAN</FONT></STRONG> </font></P>
<P align="justify"><font face="Comic Sans MS"><%=D%> (cm) �ap�nda, <%=L%> (cm) uzunlu�unda bir silindirik metal eleman ba�lang��ta 100�C'de 
  iken 50�C'de bulunan <%=V%> (m/s) h�z ile akan suyun i�ine b�rak�lmaktad�r. Metal eleman�n 
  s�cakl���n�n 75�C'ye inmesi i�in ge�en s�reyi hesaplay�n�z Eleman ile s�v� aras�ndaki 
  ta��n�m �s� transfer katsay�s�n�n hesaplanmas� s�ras�nda a�a��daki ifade kullan�lacakt�r. 
  Film �zellikler i�in ortalama 82.22�C al�nabilmektedir. </font></P>
<P align="justify"><font face="Comic Sans MS"><img src="/images/HilpertKnudsenKatz.gif" width="138" height="57" align="absmiddle"></font></P>
<P align=justify style="line height=200%"><font face="Comic Sans MS">Metal i�in 
  k= 500 (W/m�C),Cp=320 (J/kg�C) ,Ro = 8500 (kg/m<sup>3</sup>)</font></P>
<P align=center><font face="Comic Sans MS"><STRONG>CEVAPLAMA B�L�M�</STRONG></font></P>
<FORM action="<%=response.encodeURL("silindirikmetaleleman.jsp")%>" method="POST">
  <P align=center> 
  <TABLE style="HEIGHT: 61px" cellSpacing=1 cellPadding=1 width=379 
border=1>
    <TBODY bgColor=aqua>
      <TR> 
        <TD width=184><font face="Comic Sans MS">Ge�en s�re=</font></TD>
        <TD width="144" align=middle><font face="Comic Sans MS">
          <INPUT name=useranswer0>
          </font></TD>
        <TD width=33><font face="Comic Sans MS">(sn)</font></TD>
      </TR>
      <TR> 
        <TD colSpan=3> <P 
...
class Grade
{
    void GradeIt(JspWriter out,String[] results)
 {
      ...
      double vartime=Double.valueOf(dbc.getSelectedCell(username,"SystemAnswer1",TableName)).doubleValue ();
      ...
      try{
        if(!isanswered){
          ...
          //
          lowerlimit[0]=vartime*(1-QuesRange);upperlimit[0]=vartime*(1+QuesRange);
          //
