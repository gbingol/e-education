...
<%!
   ...
   final String TableName="3_SOMINE";
...
<%
  ...
  if(!IsValuesGiven){
    ...
    double Temp,Temp2,Temp3,Nu,h,Height,WidthCam;
       do{
        ...
        h = Nu * kf / Height;
        q=h * A * (Tcam-Toda);
       values[0]=Camyukseklik;values[1]=Camgenislik;values[2]=Tcam;values[3]=Toda;
      //
	  ...
	  try
		...
		outData .writeChars("k="+String.valueOf(kf)+"\n");
		outData .writeChars("Kinematik viskozite="+String.valueOf(KinematikViskozite)+"\n");
		outData .writeChars("Grashof="+String.valueOf(Gr)+"\n");
		outData .writeChars("Rayleigh="+String.valueOf(Ra)+"\n");
  ...
  dbc.close();
  Camyukseklik=values[0];Camgenislik=values[1];Tcam=values[2];Toda=values[3];
}
...
%>
<P align=center><font face="Comic Sans MS"><STRONG><FONT size=5 color="#ff0080">��M�NE</FONT></STRONG> 
  </font></P>
<P align="justify"><font face="Comic Sans MS">Bir ��mine �n�ne k�v�lc�m s��ramas�n� 
  ve odaya duman ka�mas�n� �nleme �zere <%=Camyukseklik%> (m) y�ksekli�inde, <%=Camgenislik%> (m) geni�li�inde cam yerle�tirilmi�tir. Cam�n y�zey s�cakl��� bu �ekilde <%=Tcam%>�C'ye y�kseltilmektedir. E�er oda s�cakl��� <%=Toda%>�C ise, camdan odaya �s� kayb� ne kadard�r? I��n�m�n etkisi ihmal 
  edilecektir. ��z�m s�ras�nda Churchill ve Chu e�itliklerinden yararlan�lacakt�r.</font></P>
<P align=center><font face="Comic Sans MS"><STRONG>CEVAPLAMA B�L�M�</STRONG></font></P>
<FORM action="<%=response.encodeURL("somine.jsp")%>" method="POST">
  <P align=center> 
  <TABLE style="HEIGHT: 61px" cellSpacing=1 cellPadding=1 width=379 
border=1>
    <TBODY bgColor=aqua>
      <TR> 
        <TD width=184><font face="Comic Sans MS">Is� kayb� =</font></TD>
