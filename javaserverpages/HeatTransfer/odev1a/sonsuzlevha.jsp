...
<%!
   ...
   final String TableName="1a_SonsuzLevha";
...
%>
<%
  double T1=0,T2=0,q_A=0;
  boolean IsValuesGiven=dbc.IsValueTaken("IsValueTaken","STUDENTNUMBER",TableName,username);
  if(!IsValuesGiven){
      T1 =(double)(int)(Math.random() * 200 + 600);   //1. Levhan�n s�cakl���
      T2 =(double)(int)(Math.random() * 100 + 200);   //2. Levhan�n S�cakl���
      q_A=5.669*Math.pow(10,-8)*(Math.pow((T1+273.15),4)-Math.pow((T2+273.15),4));  //W/m2
	  q_A=q_A/1000.0;  //kW/m2
   ...
   dbc.updateString("STUDENTNUMBER",username,"SystemAnswer1",TableName,String.valueOf(q_A));
 }else{
   for(int i=0;i<values.length;i++){
...
<P align=center><STRONG><FONT size=5 color="#ff0080">Sonsuz Levhalardan I��n�m 
  Is� Transferi</FONT></STRONG> </P>
<P align=justify style="line height=200%"><%=T1%>�C ve <%=T2%>�C s�cakl���nda bulunan iki sonsuz levha aras�nda radyasyon ile 
  �s� transferi ger�ekle�mektedir. Birim alandan olan �s� transferini hesaplay�n�z.</P>
<P align=center><STRONG>CEVAPLAMA B�L�M�</STRONG></P>
<FORM action="<%=response.encodeURL("sonsuzlevha.jsp")%>" method="POST">
<P align=center>
<TABLE width="40%" cellSpacing=1 cellPadding=1  border=1>
  <TBODY bgColor=aqua>
  <TR>
        <TD width=95><font face="Comic Sans MS">Is� Transferi</font> Miktar�=</TD>
    <TD width="144" align=middle><INPUT name=useranswer0></TD>
        <TD width=93>(kW/m<sup>2</sup>)</TD>
      </TR>
  <TR>
    <TD colSpan=3>
      <P 
