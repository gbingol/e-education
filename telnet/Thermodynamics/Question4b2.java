...
//***************************7th QUESTION*******************************
//
class Question4b2 extends Questions{
    ...
    if(str.equals("ValueIsGiven")) IsValuesGiven=true;
      if(!IsValuesGiven){
        try{
          Tevaporator=Double.valueOf(mhtable.getCellValue(7,1)).doubleValue ();
          Kizdirma=Double.valueOf(mhtable.getCellValue(7,2)).doubleValue ();
          Tyogunlastirici=Double.valueOf(mhtable.getCellValue(7,3)).doubleValue ();
          AsiriSogutma=Double.valueOf(mhtable.getCellValue(7,4)).doubleValue ();
    ...
    protected void CalculateWriteResults(){
      double T1,P1,h1,P3,T3,h3,hf3,T4,hf4,P4,hg4,h4,x4,s1,sg1,s2,T2,P2,h2,Qyog,Qevap,Wkomp,COP;
      T3=Tyogunlastirici;
      ...
      h4=h3-AsiriSogutma;
      x4=(h4-hf4)/(hg4-hf4);
      h1=doymus134a.Hg(T4)+Kizdirma; //In superheated region
      double temph,tempT=T4;
      do{
        tempT=tempT+0.1;
        temph=kizgin134a.h(P1/1000,tempT);
      }while(!(Math.abs(temph-h1)<5));
      double XX=tempT;double YY=tempT+10;
      double ort;
      do{
        ort=(XX+YY)/2;
        temph=kizgin134a.h(P4/1000,ort);
        if((temph-h1)>0) YY=ort;
        if((temph-h1)<0) XX=ort;
      }while(!(Math.abs(temph-h1)<0.0001));
      T1=ort;
      s1=kizgin134a.S(P1/1000,T1);
      s2=s1;
    ...
    public void GradeIt(DataOutputStream out,BufferedReader in,PrintWriter pw){
      ...
      try{
        if(!isanswered){
          ...
          mhtable.setCellNote(rownumber,15,"Kullanýcýnýn bu soru icin toplam notu");
          //
