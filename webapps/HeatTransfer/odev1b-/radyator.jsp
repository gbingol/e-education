...
<%!
   ...
   final String TableName="1b_Radyator";
...
<%
  ...
  if(!IsValuesGiven){
      ...
      values[0]=x;values[1]=k;values[2]=h1;values[3]=h2;
      //
      //
   ...
   dbc.updateString("STUDENTNUMBER",username,"SystemAnswer1",TableName,String.valueOf(K));
 }else{
   for(int i=0;i<values.length;i++){
  ...
  dbc.close();
  x=values[0];k=values[1];h1=values[2];h2=values[3];
}
%>
...
<P align=center><STRONG><FONT size=5 color="#ff0080">Radyat�r</FONT></STRONG> </P>

<P align=justify style="line height: 200%"> D�kme demirden bir radyat�r�n kal�nl��� <%=x%>(cm), �s�l iletkenlik katsay�s� <%=k%> (W/m�C) olup, radyat�r�n i� ve d�� ta��n�m �s� transfer katsay�lar� s�ra ile <%=h1%> (W/m<sup>2</sup>�C), <%=h2%> (W/m<sup>2</sup>�C) olarak verilmi�tir.
Toplam �s� transfer katsay�s�n� (W/m<sup>2</sup>�C) hesaplay�n�z.
</P>

<P align=center><STRONG><FONT face="Comic Sans MS">CEVAPLAMA B�L�M�</font></STRONG></P>
<FORM action="<%=response.encodeURL("radyator.jsp")%>" method="POST">
<P align=center>
<TABLE cellSpacing=1 cellPadding=1 width=390 border=1>
  <TBODY bgColor=aqua>
  <TR>
        <TD width=156><font face="Comic Sans MS">Toplam �s� transfer katsay�s�</font>=</TD>
    <TD width="144" align=middle><INPUT name=useranswer0></TD>
        <TD width=72>(W/m<sup>2</sup><font face="Comic Sans MS">�C</font>)</TD>
      </TR>
  <TR>
    <TD colSpan=3>
      <P 
...
class Grade
{
    void GradeIt(JspWriter out,String[] results)
 {
      ...
      double varK=Double.valueOf(dbc.getSelectedCell(username,"SystemAnswer1",TableName)).doubleValue ();
      ...
      try{
        if(!isanswered){
          ...
          //
          lowerlimit[0]=varK*(1-QuesRange);upperlimit[0]=varK*(1+QuesRange);
          //
