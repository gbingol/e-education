...
<%!
   ...
   final String TableName="1a_Conduction1";
...
<%
  ...
  if(!IsValuesGiven){
      ...
      values[0]=x;values[1]=T1;values[2]=T2;values[3]=k;
      //
      //
  ...
  dbc.close();
  x=values[0];T1=values[1];T2=values[2];k=values[3];
}
...
<P align=center><STRONG><FONT size=5 color="#ff0080">Bak�r Levha</FONT></STRONG> </P>

<P align=justify style="line height: 200%"> <%=x%> (cm) kal�nl���ndaki bak�r levhanin bir y�z� <%=T1%>�C'de, di�er y�z� <%=T2%>�C'de sabit tutulmaktad�r. Levha boyunca transfer edilen �s�y� (MW/m<sup>2</sup>) hesaplayan�z.
Levhan�n �s� iletim katsay�s� <%=k%> (W/m�C)'dir.
</P>

<P align=center><STRONG><FONT face="Comic Sans MS">CEVAPLAMA B�L�M�</font></STRONG></P>
<FORM action="<%=response.encodeURL("bakirlevha.jsp")%>" method="POST">
<P align=center>
<TABLE cellSpacing=1 cellPadding=1 width=390 border=1>
  <TBODY bgColor=aqua>
  <TR>
        <TD width=156><font face="Comic Sans MS">Transfer edilen �s�</font>=</TD>
    <TD width="144" align=middle><INPUT name=useranswer0></TD>
    <TD width=72>(MW/m<sup>2</sup>)</TD></TR>
  <TR>
    <TD colSpan=3>
      <P 
