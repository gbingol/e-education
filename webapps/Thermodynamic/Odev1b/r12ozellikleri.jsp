...
<%!
   ...
   final String TableName="Odev1b_2";
...
%>
<%
  double Vtoplam=0,P=0,m=0,T=0,x=0,h=0,Vbuhar=0;
  boolean IsValuesGiven=dbc.IsValueTaken("IsValueTaken","STUDENTNUMBER",TableName,username);
  if(!IsValuesGiven){
  double v=0,vf=0,vg=0,hf=0,hg=0,mg=0,_Vbuhar=0;
  MHFluidsSaturated sfluid=null;
  sfluid=new DoymusR12();
		do{
          Vtoplam=(double)(int)(Math.random()*50+50);
          P=(double)(int)(Math.random()*50+100);
          m=(double)(int)(Math.random()*3+2);
          v=(Vtoplam/1000)/m;
          T=sfluid.T(P);
          vf=sfluid.vf(T);vg=sfluid.vg(T);
		}while(!(v>vf&&v<vg));
        x=(v-vf)/vg;
        hf=sfluid.hf (T);
        hg=sfluid.hg (T);
        h=hf+x*(hg-hf);
        mg=x*m;
        _Vbuhar=mg*sfluid.vg(T);		//  m3/kg*kg = m^3
        Vbuhar=_Vbuhar*1000;			//	m3-->lt
        
		//
       values[0]=Vtoplam;values[1]=P;values[2]=m;
      //
      //
   ...
   dbc.updateString("STUDENTNUMBER",username,"SystemAnswer2",TableName,String.valueOf(Vbuhar));
 }else{
   for(int i=0;i<values.length;i++){
  ...
  dbc.close();
  Vtoplam=values[0];P=values[1];m=values[2];
}
...
%>
<P align=center><font face="Comic Sans MS"><STRONG><FONT size=5 color="#ff0080">R-12 
  Ak��kan�n�n �zellikleri</FONT></STRONG></font></P>
<P align="justify"><font face="Comic Sans MS"><%=Vtoplam%> litrelik bir kapta <%=P%> (kPa) bas�n�ta <%=m%> (kg) so�utucu ak��kan R-12 bulunmaktad�r. So�utucu ak��kan�n,</font></P>
<P align="justify"><font face="Comic Sans MS"> A) Entalpisini,<br>
  B) Buhar faz� taraf�ndan kaplanan hacimi (lt) bulunuz.</font></P>
<P align=center><font face="Comic Sans MS"><STRONG>CEVAPLAMA B�L�M�</STRONG></font></P>
<FORM action="<%=response.encodeURL("r12ozellikleri.jsp")%>" method="POST" onSubmit="return checkDigitSeperator(this)">
  <P align=center> 
  <TABLE style="HEIGHT: 61px" cellSpacing=1 cellPadding=1 width=379 
border=1>
    <TBODY bgColor=aqua>
      <TR> 
        <TD width=184><font face="Comic Sans MS">Entalpi=</font></TD>
        <TD width="144" align=middle><font face="Comic Sans MS">
          <INPUT name=useranswer0>
          </font></TD>
        <TD width=33><font face="Comic Sans MS">kJ/kg</font></TD>
      </TR>
	  <TR> 
        <TD width=184><font face="Comic Sans MS">Kaplanan hacim=</font></TD>
        <TD width="144" align=middle><font face="Comic Sans MS">
