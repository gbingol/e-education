...
<%!
   ...
   final String TableName="Odev1b_5";
...
<%
  ...
  if(!IsValuesGiven){
		 P=(double)(int)(Math.random()*100+150);     //kPa olarak
         V=(double)(int)(Math.random()*3+2);     //m3 olarak
		//
		jspclass.JFluids.SuOzellikleri su=new jspclass.JFluids.SuOzellikleri();
        T=su.T(P);				//�C
        m=(V/su.vg(T))*1000;	//kg-->gr
		//
       values[0]=P;values[1]=V;
      //
      //
   ...
   dbc.updateString("STUDENTNUMBER",username,"SystemAnswer2",TableName,String.valueOf(m));
 }else{
   for(int i=0;i<values.length;i++){
...
%>
<P align=center><font face="Comic Sans MS"><STRONG><FONT size=5 color="#ff0080">Piston-Silindir 
  ��inde Su Buhar�</FONT></STRONG></font></P>
<P align="justify"><font face="Comic Sans MS">Bir piston-silindir d�zene�inde 
  <%=P%> (kPa) bas�n�ta, <%=V%> (m<sup>3</sup>) doymu� su buhar� bulunmaktad�r. Silindir i�indeki,</font></P>
<P align="justify"><font face="Comic Sans MS"> A) Su buhar�n�n s�cakl���n� (�C),<br>
  B) Su buhar�n�n k�tlesini (gr) hesaplay�n�z.</font></P>
<P align=center><font face="Comic Sans MS"><STRONG>CEVAPLAMA B�L�M�</STRONG></font></P>
<FORM action="<%=response.encodeURL("pistonsilindir.jsp")%>" method="POST" onSubmit="return checkDigitSeperator(this)">
  <P align=center> 
  <TABLE style="HEIGHT: 61px" cellSpacing=1 cellPadding=1 width=379 
border=1>
    <TBODY bgColor=aqua>
      <TR> 
        <TD width=184><font face="Comic Sans MS">S�cakl�k=</font></TD>
	  ...
	  <TR> 
        <TD width=184><font face="Comic Sans MS">K</font><font face="Comic Sans MS">�tle=</font></TD>
        <TD width="144" align=middle><font face="Comic Sans MS">
          <INPUT name=useranswer1>
          </font></TD>
        <TD width=33><font face="Comic Sans MS">gr</font></TD>
      </TR>
      <TR> 
        <TD colSpan=3> <P 
...
class Grade
{
    void GradeIt(JspWriter out,String[] results)
 {
	  ...
	  double varm=Double.valueOf(dbc.getSelectedCell(username,"SystemAnswer2",TableName)).doubleValue ();
      ...
      try{
        if(!isanswered){
		  ...
		  lowerlimit[1]=varm*(1-QuesRange);upperlimit[1]=varm*(1+QuesRange);
          //
